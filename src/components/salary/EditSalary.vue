...
export default {
  ...
  mounted() {
    for (let editSaKey in this.salary) {
      this.salary[editSaKey] = this.salaryData[editSaKey]
    }
  },
  ...
  methods:{
    ...
    handleEdit(){
      ...
      data.id = this.salaryData.id
      console.log(this.salary)
      this.postRequest('/api/sal/sob/update', data).then(res=>{
        if (res){
          this.$emit('cancel')
          this.$emit('loadData')
        }
